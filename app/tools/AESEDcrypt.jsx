...
export default function AESEDcrypt({route}) {
...
function Comp() {
  ...
  const encrypt = () => {
    var res = CryptoJS.AES.encrypt(input, key).toString();
    setResult(res.toString());
  };

  const decrypt = () => {
    var res = CryptoJS.AES.decrypt(input, key).toString(CryptoJS.enc.Utf8);
    setResult(res.toString());
  };
